                             ...
                             @foreach(config('translatable.locales') as $locale)
                                 <div class="col-md-4 aboutUs-edit" style="margin-top: 10px;">
                                     <div  style="text-align: @if(app()->getLocale() === 'ar' ) right @elseif(app()->getLocale() === "he") right @else left @endif" class="form-group">
                                         ...
                                         <input type="text" class="form-control" name="{{$locale}}[name]" id="{{$locale}}-name" value="{{$aboutUs->translate($locale)->name}}">
                                     </div>
                                 </div>
                                 <div class="col-md-8 aboutUs-edit">

                                     <div style="text-align: @if(app()->getLocale() === 'ar' ) right @elseif(app()->getLocale() === "he") right @else left @endif" class="form-input aboutUs-edit">
                                         <label class="col-form-label" for="exampleFormControlTextarea1">@lang('general.'.$locale.'.description')</label>
                                         <textarea   name="{{$locale}}[description]"  class="form-control" id="{{$locale}}-description" rows="3"></textarea>
                                     </div>
                                 </div>
